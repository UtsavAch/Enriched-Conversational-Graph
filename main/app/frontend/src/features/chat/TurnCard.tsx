...
const TurnCardBase = forwardRef<HTMLDivElement, Props>(
  ({ turn, active, onSelect }, ref) => {
          ...
          <div className="turn-meta-right">
            ...
            {turn.epistemic_status !== "resolved" && (
              <Badge bg={status.bg} fg={status.fg}>
                {turn.epistemic_status}
              </Badge>
            )}
            <span className="turn-date" title={turn.timestamp}>
        ...
        {(turn.grounded_by.length > 0 || turn.speech_act !== "information") && (
          <div className="turn-footer">
            ...
            {turn.grounded_by.length > 0 && (
              <span
                className="turn-grounded"
                title={turn.grounded_by.join(", ")}
              >
