...
import { useDeleteDocument, useDocumentSearch, useDocuments, useUploadDocument } from '@/api/queries';
  ...
  return (
    <div className="documents">
      ...
      <p className="inspector-lede">
        PDFs and text files retrieved alongside the conversation memory, on their own
        context budget.
      </p>

      <input
        ref={fileRef} type="file" accept=".pdf,.txt,.md"
        onChange={handleFile} className="sr-only" id="doc-upload"
      />
      <Button
        variant="primary"
      ...
      {upload.isError && (
        <p className="health-note health-warn">{(upload.error as Error).message}</p>
      )}
          ...
          {documents.map((doc) => (
            <li key={doc.id} className="doc-item">
              <div className="doc-main">
                ...
                <span className="doc-meta">{doc.id} · {doc.n_chunks} chunks · {doc.source_type}</span>
              </div>
              <button
                className="doc-remove" aria-label={`Remove ${doc.title}`}
      ...
      {Boolean(documents?.length) && (
        <>
          ...
          <p className="field-note" style={{ marginBottom: 6 }}>
            Preview what the retriever returns for a query. Useful when a grounded answer
            looks wrong and you need to know whether retrieval or generation was at fault.
          </p>
          <div className="doc-search">
            ...
            />
            <Button size="sm" onClick={() => setSearching(true)}>Search</Button>
          </div>
          ...
          {hits?.map((hit) => (
            <div key={hit.chunk_id} className="doc-hit">
              ...
              <p className="doc-hit-source">
                {hit.source_title}{hit.page !== null ? `, p.${hit.page}` : ''}
              </p>
