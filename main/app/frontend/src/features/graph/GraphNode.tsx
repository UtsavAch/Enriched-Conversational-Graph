...
function GraphNodeImpl({
  ...
  useEffect(() => {
    ...
    const behavior = drag<SVGGElement, unknown>()
      .on("start", (event) => {
        ...
        onDrag(node.id, event.x, event.y, "start");
      })
  ...
  if (node.kind === "interaction") {
    stroke = NODE_COLOR.interaction;
    fill = NODE_FILL.interaction;
  } else if (node.kind === "entity") {
    stroke = NODE_COLOR.entity;
    fill = NODE_FILL.entity;
  } else {
  ...
  const label =
    node.kind === "interaction"
      ? node.data.summary || node.data.question
      : node.kind === "entity"
        ? node.data.name
        : node.data.label;
